...
Item {
    ...
    Text {
        ...
        y: 28
        font.pointSize: 14
        visible: false //recommentView.voiceControlled
    }

    AnimatedItem {
        id: disconnectConsole
        state: (!recommentView.voiceControlled || mainView.state == "disconnected") ? "hidden" : "shown"
    ...
    ConnectionSettings {
        ...
        state: (recommentView.voiceControlled && (mainView.state == "disconnected")) ? "shown" : "hidden"
        x: 150
        anchors.margins: 30
        width: 300
    ...
    ActiveConsole{
        anchors.fill: parent
        state: (!recommentView.voiceControlled || mainView.state == "activated") ? "shown" : "hidden"
