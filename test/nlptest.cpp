...
{
  if (!recomment.init())
    QFAIL("Failed to initialize Recomment; Aborting");
}
void NLPTest::testNLP()
{
  ...
  recomment.reset();
  QStringList resultCritiques = recomment.critique(input).trimmed().split('\n', QString::SkipEmptyParts);
...
//   QSKIP("Just NLP tests for now", SkipAll);
  ...
  recomment.reset();
  QStringList promptCritiques = recomment.critique(prompt).trimmed().split('\n', QString::SkipEmptyParts);
  recomment.reset();
  QStringList resultCritiques = recomment.critique(result).trimmed().split('\n', QString::SkipEmptyParts);
